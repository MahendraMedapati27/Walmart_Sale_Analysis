...
alter table sales add column day_name varchar(25) not null;
...
-- what is the most common payement method ?
...
alter table sales add column review varchar(15) not null;
SET @avg_total = (SELECT AVG(total) FROM sales);
UPDATE sales
SET review = (
    CASE
        WHEN total > @avg_total
        THEN 'Good'
        ELSE 'Bad'
    END
);
